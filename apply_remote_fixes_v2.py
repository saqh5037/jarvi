...
export default {
  ...
  BASE_URL
        ...
        # Write api.js config
        cmd = "cd " + REMOTE_PATH + " && mkdir -p src/config && cat > src/config/api.js << 'EOF'\n" + api_config + "\nEOF"
        ...
        with SCPClient(ssh.get_transport()) as scp:
            ...
            remote_file = REMOTE_PATH + "/src/components/DashboardStatsDynamic.jsx"
            try:
        ...
        update_cors = '''cd ''' + REMOTE_PATH + '''
for file in server-enhanced-notes.js server-meetings.js server-tasks.js server-voice-notes.js; do
  if [ -f "$file" ]; then
    # Update WebSocket CORS to allow all origins
    sed -i '' 's/"http:\\/\\/localhost:5173"/true/g' "$file"
    sed -i '' 's/\\["http:\\/\\/localhost:5173"\\]/true/g' "$file"
    ...
    echo "Updated $file"
        ...
        update_app = '''cd ''' + REMOTE_PATH + '''/src
if [ -f "App.jsx" ]; then
  # Add import for DashboardStatsDynamic if not exists
  if ! grep -q "DashboardStatsDynamic" App.jsx; then
...
ps aux | grep node | grep -v grep | head -5'''
        ...
        cmd = "cd " + REMOTE_PATH + " && cat > start_fixed.sh << 'EOF'\n" + startup_script + "\nEOF"
        execute_command(ssh, cmd)
        execute_command(ssh, "cd " + REMOTE_PATH + " && chmod +x start_fixed.sh")
        
        # Start services
        execute_command(ssh, "cd " + REMOTE_PATH + " && ./start_fixed.sh", "Starting all services...")
